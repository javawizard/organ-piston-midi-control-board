  ...
  public:
    ...
    void loop() {
      ...
      for (int i = 0; i < PINS_PER_KEYBOARD; i++) {
        pistons[i].update_with_new_value(midi_channel, midi_offset + i, !(pin_states & 1), led_color);
...
Keyboard keyboards[] = {
  Keyboard(0x20, 0x21, 0, 0, 0x0000FF), // choir
  Keyboard(0x22, 0x23, 0, 32, 0x00FF00), // great
  // Keyboard(0x24, 0x25, 0, 64, 0xFFFF00), // swell
  // Keyboard(0x26, 0x27, 0, 96, 0xFF0000), // solo
  Keyboard(0x60, 0x61, 1, 0, 0x00FFFF), // pedal left (translated through an LTC4316 on the pedal controller)
  // Keyboard(0x62, 0x63, 1, 32, 0xFF00FF) // pedal right (translated through an LTC4316 on the pedal controller)
};
...
void setup() {
  USB.productName("Organ Piston MIDI Controller (by javawizard weeeeee)");
  USB.manufacturerName("github.com/javawizard");
  USB.VID(0x1234);
  tinyusb_enable_interface(USB_INTERFACE_MIDI, TUD_MIDI_DESC_LEN, load_midi_usb_descriptor);
  USB.begin();
